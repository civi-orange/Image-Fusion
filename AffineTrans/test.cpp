...
int main111() {
	...
	int dst_sa_rows = round(fabs(dst_sc_rows * cos(angle)) + fabs(dst_sc_cols * sin(angle)));//再经过旋转后图像高度
	int dst_sa_cols = round(fabs(dst_sc_cols * cos(angle)) + fabs(dst_sc_rows * sin(angle)));//再经过旋转后图像宽度
	
	double Angle = angle * CV_PI / 180.0;
	double alpha = cx * cos(Angle);
	double beta = cy * sin(Angle);
	cv::Point2f center = cv::Point2f(src.cols / 2, src.rows / 2);
	cv::Mat OKK_mat = (cv::Mat_<double>(2, 3) << alpha, beta, ((1.0 - alpha) * center.x - beta * center.y),
												-beta, alpha, (beta * center.x + (1.0 - alpha) * center.y));

	cv::Mat tempMat_r = cv::getRotationMatrix2D(center, angle, 1);	
	...
	cv::Mat yyyy;
	warpAffine(src, yyyy, tempMat_t, src.size()+cv::Size(100,100));
	imshow("1234", yyyy);
	
	warpAffine(src, dst1, OKK_mat, src.size());
	warpAffine(src, dst2, tempMat_r, src.size());
	warpAffine(dst2, dst3, tempMat_c, src.size());
	//warpAffine(dst3, dst4, tempMat_t, cv::Size(dst_sa_cols, dst_sa_rows));
	clock_t time_e1 = clock();
	std::cout << "running time: " << time_e1 - time_s1 << "ms" << std::endl;
	imshow("dst1", dst1);
	imshow("dst2", dst2);
	imshow("dst3", dst3);
	//imshow("dst4", dst4);	
	imwrite("111.jpg", dst2);
